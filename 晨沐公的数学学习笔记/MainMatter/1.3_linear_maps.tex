...
\subsection{用矩阵描述向量的关系}

设线性映射$A$的表示矩阵$\begin{pmatrix}
	v_1 & \cdots & v_n
\end{pmatrix}=\begin{pmatrix}
	u_1^{\T} & \cdots & u_n^{\T}
\end{pmatrix}^{\T}$. 一方面, 容易证明$\spn (v_1,\cdots ,v_n) = \rge A$. 因此, 我们常称$\rge A$为矩阵$A$的\textit{列空间}. 另一方面, 亦容易验证矩阵$A^{\T}$所对应的线性映射$A'$满足$\spn (u_1^{\T} , \cdots , u_n^{\T}) = \rge A'$. 
...
\begin{example} %LADR 练习3.20,21
	(1) 设$W$是有限维的, $T \in \lmap (V,W)$. 证明$T$是单的当且仅当存在$S \in \lmap (W,V)$使得$ST$是$V$上的恒等映射. \\
	(2) 设$V$是有限维的, $T \in \lmap (V,W)$. 证明$T$是满的当且仅当存在$S \in \lmap (W,V)$使得$TS$是$W$上的恒等映射.
\end{example}
